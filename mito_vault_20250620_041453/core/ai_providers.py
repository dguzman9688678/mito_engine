...
Respond as MITO would - knowledgeable, autonomous, and ready to assist with development tasks."""
        ...
        payload = {
            "model": os.getenv("LLAMA_MODEL_NAME", "llama-3.3-70b-versatile"),
            "messages": [
...
def ai_generate(prompt: str, provider: Optional[str] = None, session_id: str = None) -> str:
    ...
    try:
        ...
        if session_id:
            context_prompt = memory_manager.build_context_prompt(session_id, prompt)
            # Store user message
            memory_manager.store_message(session_id, "User", prompt, importance=1)
    ...
    logger.info(f"AI generation request - Provider: {provider}, Prompt length: {len(prompt)}")
    ...
    if memory_manager and session_id and not result.startswith("Error:"):
        try:
            memory_manager.store_message(session_id, "MITO", result, importance=1)
        except Exception as e:
...
def get_available_providers() -> dict:
    ...
    providers = {
        "openai": {
            ...
            "model": "gpt-3.5-turbo",
            "status": "configured" if os.getenv("OPENAI_API_KEY") else "missing_api_key"
        },
        "llama": {
            "name": "LLaMA 3",
        ...
        "claude": {
            ...
            "model": os.getenv("CLAUDE_MODEL_NAME", "claude-3-opus-20240229"),
            "status": "configured" if os.getenv("CLAUDE_API_KEY") else "missing_api_key"
        },
        "local": {
            "name": "Local Fallback",
            "available": True,
